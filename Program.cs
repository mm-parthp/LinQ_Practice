...
namespace LinQ_Practice
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //WHERE
            var querySyntax0 = from employee in employees
                               where employee.EmployeeDes.EndsWith("g")
                               select employee.EmployeeDes;
            Console.WriteLine("Where in querySyntax------");
            foreach (var item in querySyntax0)
            {
                Console.WriteLine(item);
            }
            var methodSyntax0 = employees.Where(e => e.EmployeeDes.StartsWith("d"));
            Console.WriteLine("Where in methodSyntax-----");
            foreach (var item in methodSyntax0)
            {
                Console.WriteLine(item.EmployeeDes);
            }
            Console.WriteLine('\n');
